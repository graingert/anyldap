...
- ``zope.interface`` is no longer a dependency. The ``I*`` interfaces in
  ...
  methods rather than whatever was declared to. What ``@implementer`` said
  is said by inheriting the protocol, which is checked rather than
  recorded: its members are abstract, so a class that claims one and does
  not fill it is refused when it is instantiated, and mypy says so at
  every place that instantiates it. ``I.providedBy(x)`` is
  ``isinstance(x, I)``. The one place that adapted
