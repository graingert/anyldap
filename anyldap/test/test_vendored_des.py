"""Vendored from passlib 1.7.4's passlib/tests/test_crypto_des.py.
...
from anyldap.samba._passlib.des import (
    _KDATA_MASK,
    _KPARITY_MASK,
    INT_56_MASK,
    INT_64_MASK,
...
class TestDes:
    ...
    def test_03_encrypt_bytes(self) -> None:
        ...
        for key, plaintext, correct in self.des_test_vectors:
            ...
            for _ in range(20):
                key3 = _pack64(self._random_parity(rng, _unpack64(key)))
    ...
    def test_04_encrypt_ints(self) -> None:
        ...
        for key, plaintext, correct in self.des_test_vectors:
            ...
            # test with random parity bits
            for _ in range(20):
                key3 = self._random_parity(rng, key)
